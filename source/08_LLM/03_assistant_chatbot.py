# 자동완성이 안 될 경우 (가상환경 설정 안되있는 경우 ) : ctrl+shift+p -> 가상환경선택
...
assistant_cs = client.beta.assistants.create(
    ...
    instructions="당신은 고객 지원 챗봇입니다. 사용자 문의에 대해 30자이내로 친절한 답변을 하세요",
    model = "gpt-4o-mini"
)
# 3. thread 생성 : 기억담당
thread_cs = client.beta.threads.create()
print("챗봇이 시작됩니다. (종료:'종료'나 'exit'). 모든 대화는 저장 됩니다.")
...
with open('data/ch7_chat_history.txt', 'w', encoding='utf-8') as f:
    for message in sorted_messages:
        # 생성 시각(message.createc_at)을 datetime으로 변환
