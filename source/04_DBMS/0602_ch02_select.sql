...
SELECT EMPNO NO, ENAME, SAL FROM EMP WHERE SAL!=3000;
    ...
    -- EX4) 이름(ENAME)이 FORD인 직원의 EMPNO, ENAMEM, MGR(상사의 사번)을 출력
...
-- 4. 특정 행 출력 : WHERE절(조건절) - 논리연산자 : AND, OR, NOT
    -- EX1. RMQDU(SAL)가 2000이상 3000이하인 직원의 모든 필드
    ...
    -- EX5. JOB이 MANAGER이거나 10번 부서(DEPTNO) 직원의 모든 필드
...
SELECT EMPNO, ENAME, JOB, HIREDATE
    FROM EMP
    WHERE TO_CHAR(HIREDATE, 'YY/MM/DD')>'81/01/31';
...
-- 8. SQL연산자(BETWEEN, IN, LIKE, IS NULL)
    -- (1) BETWEEN A AND B : A부터 B까지(A,B포함)
        -- EX1. SAL이1500이상 300이하인 직원의 사번, 이름, 급여
...
SELECT * FROM EMP ORDER BY SAL DESC, HIREDATE; -- SAL기준 내림차순 (SAL이 같으면 HIREDATE)

-- 연습문제 전 형변환 함수 : TO_CHAR, TO_DATE
-- ※ 날짜형(HIREGATE)을 문자형으로 변환 : TO_CHAR(날짜형데이터, 패턴)
...
SELECT EMPNO, ENAME, HIREDATE, JOB, SAL
    FROM EMP
    WHERE TO_CHAR(HIREDATE, 'YYYY') = '1981' AND JOB <> 'SALESMAN';
