...
문자 : char(n), text, mediuntext(16MB), longtext(4GB)
날짜 : date, datetime, time, year, timestamp
*/
-- DDL이나 DML명령어는 데이터 베이스 내에서 실행
...
insert into student values (102, '홍길동', 2);
insert into student values (103, '홍길동', 3);
insert into student values (104, '홍길동', 4);
select * from student;
select * 
...
select w.pno, w.pname, m.pname, dname
    ...
    where w.manager=m.pno && w.dno=m.dno;
...
select * from personal;
select pname, pay, bonus, ifnull(bonus, 0) bonus from personal;
